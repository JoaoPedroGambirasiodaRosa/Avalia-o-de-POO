...
public class AgendaDAO {
    ...
    public void create(Agenda u) throws NoSuchAlgorithmException {
        PreparedStatement stmt = null;
        BigInteger cripto;
        try {
    ...
    public ArrayList<Agenda> read() {
        PreparedStatement stmt = null;
        ResultSet rs = null;
        List<Agenda> usuarios = new ArrayList<Agenda>();
        try {
            stmt = con.prepareStatement("SELECT * FROM tbagenda ORDER BY nome ASC");
            rs = stmt.executeQuery();
            while (rs.next()) {
                Agenda usuario = new Agenda();
        ...
        return (ArrayList<Agenda>) usuarios;
    }
